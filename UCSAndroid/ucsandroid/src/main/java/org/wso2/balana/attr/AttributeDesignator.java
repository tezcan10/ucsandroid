  ...
  extends AbstractDesignator
{
    ...
    throws ParsingException
  {
    ...
    {
      int target;
      if (tagName.equals("ResourceAttributeDesignator"))
      {
        target = 1;
      }
      else
      {
        int target;
        if (tagName.equals("ActionAttributeDesignator"))
        {
          target = 2;
        }
        else
  ...
  public void encode(OutputStream output, Indenter indenter)
  {
    ...
    if ((target == 0) && (category != null)) {
      tag = tag + " SubjectCategory=\"" + category.toString() + "\"";
    }
    tag = tag + " AttributeId=\"" + id.toString() + "\"";
    tag = tag + " DataType=\"" + type.toString() + "\"";
    if (issuer != null) {
      tag = tag + " Issuer=\"" + issuer.toString() + "\"";
    }
    if (mustBePresent) {
