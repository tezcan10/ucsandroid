...
class Base64 {private static final char SPACE = ' ';
  ...
  public static String encode(byte[] binaryValue) {
    ...
    for(int binaryValueNdx = 0; binaryValueNdx < binaryValueLen; binaryValueNdx += 3) {
      int group1 = binaryValue[binaryValueNdx] >> 2 & 63;
      sb.append("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/= \u0004\u000b\f\t\n\u0013\r".charAt(group1));
      int group2 = binaryValue[binaryValueNdx] << 4 & 48;
      if (binaryValueNdx + 1 < binaryValueLen) {
        group2 |= binaryValue[binaryValueNdx + 1] >> 4 & 15;
      }

      sb.append("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/= \u0004\u000b\f\t\n\u0013\r".charAt(group2));
      int group3;
      if (binaryValueNdx + 1 < binaryValueLen) {
        group3 = binaryValue[binaryValueNdx + 1] << 2 & 60;
      ...
      int group4;
      if (binaryValueNdx + 2 < binaryValueLen) {
        group4 = binaryValue[binaryValueNdx + 2] & 63;
      } else {
        group4 = 64;
      }
  ...
  private static void initDecodeArray() {
    if (Base64DecodeArray == null) {
      ...
      for(int i = 0; i <= 122; ++i) {
        ourArray[i] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/= \u0004\u000b\f\t\n\u0013\r".indexOf(i);
      }
      ...
      if (decodedChar == -1) {
        ...
        switch(state) {
          case 0:
            ...
            quantum[0] = (byte)(decodedChar << 2 & 252);
            state = 1;
            break;
          case 1:
            if (decodedChar == 64) {
            ...
            quantum[1] = (byte)(decodedChar << 4 & 240);
            state = 2;
            break;
          case 2:
            if (decodedChar == 64) {
              ...
              quantum[1] = (byte)(quantum[1] | decodedChar >> 2 & 15);
              quantum[2] = (byte)(decodedChar << 6 & 192);
              state = 3;
            }
            break;
          case 3:
